...
class NaturalGasAmericaCapacitySpider(scrapy.Spider):
    name = "natural_gas_america_capacity"
    allowed_domains = ["pipeline2.kindermorgan.com"]
    start_url = "https://pipeline2.kindermorgan.com/Capacity/OpAvailSegment.aspx?code=NGPL"
    mongo_collection = "ebb_natural_gas_america_capacity"
    ...
    def parse_export(self, response):
        ...
        if body[:2] == b"PK":
            ...
            self.logger.warning(
                "Unsupported Natural Gas America capacity export format content_type=%s url=%s",
                content_type,
                response.url,
            )
            yield {
        ...
        if not rows:
            self.logger.warning("No rows found in Natural Gas America capacity export")
            return
        ...
        for row in rows[1:]:
            ...
            if "TSP_Name" not in item:
                item["TSP_Name"] = "Natural Gas Pipeline Company of America LLC"

            if item.get("Loc") and item.get("Post_Date"):
                yield item
