...
class ColumbiaGasTransmissionCapacitySpider(scrapy.Spider):
    name = "tco_capacity"
    allowed_domains = ["ebb.tceconnects.com"]
    custom_settings = {"DOWNLOAD_TIMEOUT": 120}
    mongo_collection = "ebb_columbia_gas_transmission_capacity"
    ...
    report_urls = {
        "Operationally Available": (
            ...
            "?/InfoPost/OperationallyAvailableCapacity&pAssetNbr=51"
        ),
        "Unsubscribed": (
            "https://ebb.tceconnects.com/infopost/ReportViewer.aspx"
            "?%2fInfoPost%2fUnsubscribedCapacity&AssetNbr=51"
        ),
    }
    ...
    def parse_capacity_report(self, response):
        ...
        if not yielded:
            self.logger.warning(
                "TCO capacity report did not yield parseable table rows for %s: %r",
                report_type,
                response.text[:200],
            )
