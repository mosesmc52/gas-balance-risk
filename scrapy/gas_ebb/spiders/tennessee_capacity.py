...
class TennesseeCapacitySpider(scrapy.Spider):
    name = "tenn_capacity"
    allowed_domains = ["pipeline2.kindermorgan.com"]
    start_url = "https://pipeline2.kindermorgan.com/Capacity/OpAvailSegment.aspx?code=TGP"
    mongo_collection = "ebb_tennessee_capacity"
    ...
    def parse_export(self, response):
        ...
        if body[:2] == b"PK":
            ...
            self.logger.warning(
                "Unsupported Tennessee capacity export format content_type=%s url=%s",
                content_type,
                response.url,
            )
            yield {
        ...
        if not rows:
            self.logger.warning("No rows found in Tennessee capacity export")
            return
        ...
        for row in rows[1:]:
            ...
            if "TSP_Name" not in item:
                item["TSP_Name"] = "TENNESSEE GAS PIPELINE"
