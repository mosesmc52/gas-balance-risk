...
class LouisianaCapacitySpider(scrapy.Spider):
    name = "louisiana_capacity"
    allowed_domains = ["pipeline2.kindermorgan.com"]
    start_url = "https://pipeline2.kindermorgan.com/Capacity/OpAvailSegment.aspx?code=KMLP"
    mongo_collection = "ebb_louisiana_capacity"
    ...
    def parse_export(self, response):
        ...
        if body[:2] == b"PK":
            ...
            self.logger.warning(
                "Unsupported Louisiana capacity export format content_type=%s url=%s",
                content_type,
                response.url,
            )
            yield {
        ...
        if not rows:
            self.logger.warning("No rows found in Louisiana capacity export")
            return
        ...
        for row in rows[1:]:
            ...
            if "TSP_Name" not in item:
                item["TSP_Name"] = "KM LOUISIANA PIPELINE"
